...
- 2026-08-24: Corrected scheduled-prompt step 12 so its one historical backfill uses the same strict Spark-first/Luna-low coded-availability policy, including a permitted same-run Luna latch; if both allowed workers fail, the verified link and prior manifest remain current. A focused content-safe test prevents restoration of the stale skip/switch-model wording. This corrective prompt, routing/docs code, and test await commit/push; synchronizing the saved task UI is blocked until a signed-in Chrome profile is available.
