...
| `PERF-002` | **implemented; release gate in progress** | Warm the normal daily path without creating an unbounded Scripture archive. | Today plus seven private records paint from a reader/plan/version-bound IndexedDB snapshot. Eligible ESV passages use separate total/book/age/version enforcement; current and next warm first and missing chapters stream on open. |
...
| `AUTO-002` | **active; first unattended run pending** | Run one bounded daily T+7 preparation/publication work order. | **Prepare Daily Bible Reader T+7** runs daily at 3:00 a.m. Detroit time. `study:next` selects exactly D+7; the primary task owns the broad synthesis; Spark is Henry-only; a confirmed Spark quota failure permits only a verified full public-domain link. Content uploads precede the manifest and exact readback is mandatory. See `CONTENT_AUTOMATION.md` and `AUTOMATION_RUNBOOK.md`. |
...
| `CONTENT-002` | **live through D065; rolling task active** | Prepare later bridge syntheses only as the approved rolling buffer requires. | The initial current-plus-seven fill ends at D065. Subsequent runs may publish exactly the one reading entering T+7, never a batch or the new long-term plan. |
...
| `OFFLINE-001` | **implemented; installed-phone gate pending** | Retain as much of the current-plus-seven ESV horizon as verified terms allow. | Policy v5 uses per-passage IndexedDB records, 500-total/half-book eviction, eight-day expiry, network-first refresh, explicit partial-coverage reporting, and no service-worker Scripture caching. Full short-book assignments remain chapter-streamed and only partially retainable. |
