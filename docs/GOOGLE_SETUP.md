...
The unchanged production URL currently serves confirmed-working immutable version 19. Versions 20–22 failed the iPhone canary before the application core started; version 22 failed despite a maximum generated line of 817 characters, disproving the prior line-length hypothesis. The replacement architecture keeps production on version 19 while GitHub Pages publishes a code-only frontend and Apps Script version 23 serves as the reduced authenticated launcher/backend canary. Initial hybrid build `c57d948db8fbf838` uses frontend release `73da95f8a9ec3bb3`. Pages HTTPS readback, CORS, integrity, and exact-byte checks passed; the Google-stored version-23 files also matched the inspected build exactly. Only the installed-iPhone gate remains before promotion. Server enrollment, private Drive content, ESV retrieval, comments, scopes, execution identity, deployment access, and sharing remain unchanged. The complete incident record is in `docs/RELEASE_STABILITY.md`.
