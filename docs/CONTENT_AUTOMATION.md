...
Status: recurring generation remains proposed and not enabled. The reader warns when fewer than three consecutive substantive studies remain, showing Dustin the exact first gap and Shane only a generic delay. A read-only local evaluator models separate seven-day draft and five-day explicitly approved live buffers and selects at most one earliest gap. D057 and D058 have each completed a bounded manual work order and private publication; neither run enabled recurrence or automated publication. The narrower Henry precursor resolves today through at most two days ahead, creates private verse audits, and atomically exports an app-neutral store; by itself it does not create the broader synthesis, satisfy either readiness buffer, update Drive, publish, or recur. The normal publication handoff now resolves the checksum-bound current Henry catalog and attaches the newest reviewed artifact every time, so a background rerun cannot be hidden by an older embedded copy. Checked against the official OpenAI scheduled-tasks documentation on 2026-08-10: <https://learn.chatgpt.com/docs/automations>.
...
For the broader synthesis, the primary scheduled Codex task must retain the entire workflow: select the work order, conduct actual research, enforce rights and source status, weigh evidence, draft and edit the study, verify citations, and run deterministic validation. Do not invoke Spark for this work. The exact ChatGPT-authenticated `gpt-5.3-codex-spark` model is reserved for the high-volume verse-by-verse Matthew Henry condensation lane; the D057 main-synthesis run is retained only as a completed calibration exception.
...
4. If generation is privately enabled, emit a schema-validated work order for **exactly one** earliest missing reading and invoke `$draft-daily-commentary` with the unchanged packet in the primary task. Do not delegate its main synthesis to Spark, skip ahead, bulk-generate, or alter any other reading.
...
3. Retain the completed D057 work order, source packet, one-off Spark calibration draft, primary-task edit, coverage audit, and validation report privately; future main-synthesis trials remain entirely in the primary task.
