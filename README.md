...
The broader `$draft-daily-commentary` workflow may also use the exact authenticated `gpt-5.3-codex-spark` model as a drafting worker after the main task has issued one validated work order and prepared a Scripture-free, rights-safe source packet. Spark receives one reading, works read-only, and returns schema-constrained private output. It never selects scope, researches inaccessible sources, approves its own work, publishes, or runs in the reader.
