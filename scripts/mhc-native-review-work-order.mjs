...
async function main() {
  ...
  const report = await nativeReviewWorkOrder({root,workRoot:path.join(root,"private-content/automation/mhc-native-work-items"),privateRoot:path.join(root,"private-content"),canonicalRoot:path.join(root,"private-commentary/mhc"),libraryRoot:path.join(root,"private-commentary/mhc/stores/library"),transactionRoot:path.join(root,"private-content/automation/mhc-native-work-items/transactions"),plan,appConfig,handoffSchema,transactionSchema,approvalSchema,candidateSchema,reviewSchema,runtimeSchemaPath:path.join(trackedRoot,"schemas/mhc-runtime.schema.json")});
