...
The unchanged production URL currently serves confirmed-working immutable version 19. Versions 20 and 21 were rolled back/withheld after the iPhone showed that the independent watchdog ran but the main application did not reach its first instruction. Exact immutable-artifact comparison found 56,444- and 50,482-character minified lines in those failures versus 49,022 in working version 19. Recovery version 22 (`aa5f629de676c1b3`) is the canary: its parsed logic is identical to version 21 except for the build ID, while generated lines are capped at 817 characters. Version 19 established local-first rendering of an identity-bound saved calendar before the slow full bootstrap, followed by a lightweight identity/allowlist/enrollment/scopes/manifest check and a batched seven-reading refresh. Server enrollment, private Drive content, ESV retrieval, comments, scopes, execution identity, deployment access, and sharing remain unchanged; the restricted Sheet retains its frozen `highlight-events` tab. The server accepts commentary schemas v1, v2, and v3 while active substantive readings use v3. The complete incident matrix is in `docs/RELEASE_STABILITY.md`.
...
The local/conventional shell declares a dedicated 180-pixel Apple touch icon and 192-/512-pixel manifest icons. Google documents that favicon link tags included directly in Apps Script HTML are ignored; production therefore calls `HtmlOutput.setFaviconUrl` with the embedded 180-pixel PNG. That call is isolated from application startup. Apple documents `apple-touch-icon` as the Web Clip mechanism, while Apps Script exposes only a favicon setter, so promotion of this outer favicon to the iOS Home Screen must be verified on-device. If version 14 still receives Google's old icon after removing and re-adding the item, a stable HTTPS PNG asset or a code-only PWA shell is the remaining reliable route; either external-hosting change requires approval. References checked 2026-08-08: <https://developers.google.com/apps-script/reference/html/html-output> and <https://developer.apple.com/library/archive/documentation/AppleApplications/Reference/SafariWebContent/ConfiguringWebApplications/ConfiguringWebApplications.html>.
...
1. Run `npm run check` and inspect `dist/apps-script/`.
   Record the generated HTML byte count for regression diagnosis, but do not infer an undocumented Apps Script limit from size alone.
2. Create a new immutable Apps Script version and update the existing deployment to it; keep the same `/exec` deployment URL.
3. Do not break the `getBootstrapData(readerCode)` signature in the same release that introduces a replacement. Preserve it for at least one transition so an older client can learn the current build ID.
4. Launch the installed app online. If its deterministic client build differs from the server build, use the visible **Load latest version** action. It navigates the top-level app to the allowlisted deployment URL with a content-hash query parameter.
5. Confirm the build IDs shown by the cache inspector match, then test the full-month calendar, selection without immediate navigation, the date-specific open action, both readers' completion dots, a multi-chapter three-page flow, a placeholder flow, comment creation from each page position, **Finished** returning home, and live ESV text. If iOS still presents the prior shell, fully close the Web App and open it again through the versioned action; as a last resort, remove and re-add the Home Screen app after clearing downloaded data.
6. For a local-first candidate, launch the separate canary once online and allow its seven-reading preparation to finish. Close it completely, relaunch, and verify that the saved calendar appears before the Google/Drive refresh finishes; cached commentary may open immediately, ESV must still wait for the network, and queued writes must not leave the device until access is confirmed.
7. Only after that two-launch canary check should the existing production deployment be updated. Keep the prior immutable version number and its deployment command recorded for immediate rollback.

This design aggressively detects and escapes stale versions but cannot force invisible background updates in an Apps Script iframe. If deployment testing demonstrates a need for a guaranteed service-worker-controlled shell, pause and review the public code-only PWA/GIS fallback in `docs/ARCHITECTURE.md`.
