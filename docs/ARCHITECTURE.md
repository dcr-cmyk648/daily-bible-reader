...
The August 2026 startup incident showed that URL busting alone is not a complete release strategy. Version 15's 143,455-byte generated page loaded on the iPhone; versions 16 and 17 (150,298 and 153,034 bytes) stalled before startup; restoring version 15 on the same URL immediately worked with the same current Drive records. Version 18 then reproduced the failure after minification to 98,373 bytes, with the visible phase reaching **Authorizing…**. This rules out commentary-version rejection, ordinary stale-URL behavior, and shell size as sufficient explanations. The remaining evidence points to latency or failure at the initial `google.script.run` bootstrap boundary. Production JavaScript/CSS remains minified with an exact dependency and a Safari 15 target, but the build records rather than enforces an invented byte ceiling. Startup handles an already-fired `DOMContentLoaded`, exposes a pre-storage phase, bounds IndexedDB operations, and times out Apps Script RPCs rather than waiting forever.

This is robust update recovery, not a promise of invisible background replacement. If deployment testing shows that reliable cold offline launch is essential, the documented fallback is a public code-only PWA shell with a same-origin service worker plus Google Identity Services and server-side identity validation. No private content, comments, ESV text, or credentials could enter that shell's public artifact.
