# Matthew Henry fact extraction worker — mhc-fact-extractor/v5
...
Use only the supplied normalized Matthew Henry commentary atoms and their controller-generated evidence snippets. The embedded Scripture transcription has been removed. Short `verse_anchor_terms` may help locate a target inside a shared treatment, but never reproduce or reconstruct Scripture from them.
...
- cite exactly one allowed `source_atom_id` and exactly one of that atom's supplied `source_snippet_id` values whose text supports the fact;
...
- provide one to three short `must_include_terms` copied exactly from the selected evidence snippet, with each term limited to one, two, or three words; and
...
Copy the selected snippet text into `evidence_quote` as required by the output schema. The controller canonicalizes that field from `source_snippet_id` after generation, so the snippet ID—not your transcription—is the authoritative evidence selection.
...
Choose `qualification` from the selected evidence snippet, not from your own caution. Use `none` when the snippet asserts the fact directly; `some_understand` only when it explicitly says that some think, understand, take, make, or interpret it that way; `alternative` when it introduces a distinct option with “or,” “either,” “another,” or equivalent wording; and `uncertain` when it says may, might, perhaps, possibly, probably, uncertain, or unclear. For every non-`none` fact, include the snippet's short qualification words in `must_include_terms` so the writer cannot turn it into certainty.
