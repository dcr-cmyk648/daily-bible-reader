...
async function main(){
  ...
  const names=new Set();
  for(const entry of manifest.files){if(!/^(scripts|schemas|prompts|fixtures)\/[A-Za-z0-9._/-]+$/.test(entry.path)||entry.path.split("/").some(p=>p===".."||p==="")||names.has(entry.path))fail("invalid manifest file path");names.add(entry.path);const file=path.join(releaseRoot,entry.path);if(!file.startsWith(`${releaseRoot}${path.sep}`))fail("file escaped release");if(await realpath(file)!==file)fail("redirected release file");const bytes=await readFile(file),stat=await lstat(file);if(!stat.isFile()||stat.isSymbolicLink()||bytes.length!==entry.bytes||sha(bytes)!==entry.sha256)fail(`release file mismatch: ${entry.path}`);}
