...
The private reader is operating as a seven-day pre-launch bridge based on Celebration Church's *Reading the Bible in 3 Years — Year 3 Quarter 4*. Apps Script version 23 passed its installed-iPhone open/reopen, live-ESV, and highlight-write gate and remains the rollback launcher/backend. GitHub Pages now publishes the installable top-level PWA as well as the content-addressed frontend. Its separate immutable Apps Script version-24 bridge executes as the owner and authenticates Dustin/Shane through the existing long reader codes; it does not use Google OAuth/GIS. The Pages path is awaiting its installed-iPhone gate before it replaces the version-23 installation.
...
- The Apps Script rollback installation retains WebKit's **D** monogram. The Pages PWA supplies the intended open-Bible Apple touch icon and manifest icons; its installed-iPhone appearance remains part of the final gate.
...
- The Pages PWA still requires an installed-iPhone open/reopen, ESV, comment, and reversible highlight test before becoming the primary Home Screen installation. Version 23 remains the tested rollback until that gate passes.
...
Open the published Pages PWA on Dustin's iPhone, enter the existing Dustin reader code once, and complete the open/reopen, live ESV, comment, and reversible highlight gate while retaining the version-23 installation for rollback. After the stability decision, the first queued product change is `CAL-001` (selected-day verse preview), followed by readiness automation and the Matthew Henry-based rolling content workflow recorded in `docs/BACKLOG.md`.
