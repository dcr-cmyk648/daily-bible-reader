...
import {buildLongTermCandidate, candidateMetrics, renderCandidateReport, validateLongTermCandidate} from "./lib/long-term-schedule.mjs";
...
async function main() {
  ...
  if (check) {
    const [trackedPlan, trackedReport] = await Promise.all([readFile(OUTPUT, "utf8"), readFile(REPORT, "utf8")]);
    if (trackedPlan !== planBody || trackedReport !== reportBody) throw new Error("Generated long-term candidate or report is stale; run npm run plan:long-term:generate.");
  ...
  await Promise.all([writeAtomic(OUTPUT, planBody), writeAtomic(REPORT, reportBody)]);
