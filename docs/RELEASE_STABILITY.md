...
The Nahum readiness incident exposed a separate stale-data class rather than an Apps Script startup failure: D057 first remained `unreviewed`, and the priority warmer then replaced the valid seven-day IndexedDB placeholder without rerendering an already opened page. D057 and D058 are now `in_review` with complete reviewed Henry layers attached. The client regression-tests unconditional post-authorization revalidation for current and next, blocking refresh for an opened placeholder, in-place rerender when a different commentary version/hash arrives, and inclusion of the Henry generation timestamp in the payload revision. The private bundle also resolves the checksum-bound current Henry catalog before validation, preventing a newly reviewed background artifact from being masked by an older embedded runtime. The readiness contract audits every requested end-to-end component and starts its advance horizon with tomorrow. The public release changes no service-worker storage schema, token transport, Apps Script backend, or ESV persistence rule.
