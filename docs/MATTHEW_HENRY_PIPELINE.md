...
The deterministic `mhc-backfill-work-order/v1` queue revisits that kind of fallback after the daily T+7 study is safe. It selects at most one earliest published fallback through the Spark-first ensure contract. One Spark model-execution failure, including quota, availability, or process failure, may retry only `gpt-5.6-luna` once at low reasoning; deterministic request, source, checksum, schema, security, repository, review, and publication failures remain fail-closed. Sol, Terra, and every other model are forbidden. A successful artifact still passes complete atom-by-atom primary review before `sync-latest-mhc` replaces the fallback and the private metadata/manifest are republished atomically. The queue never changes the broader commentary synthesis.

## Native scheduled worker boundary

The scheduled backfill path uses `mhc:native:prepare`, `mhc:native:submit`, and `mhc:native:assemble` rather than nested `codex exec`. Before model-specific construction, prepare writes one checksum-bound `mhc-native-pair/v1` state for the primary quarter-hour slot. Spark and Luna therefore share one reading plus exact chapter/source/chunk identity even if the cooldown queue changes; an evidence/source preflight failure becomes a durable deterministic block rather than a Luna-eligible missed run. Prepare then creates one ignored `mhc-native-work-item/v1` directory per chapter chunk with an evidence-preserving bounded source view, copied schemas, rendered instructions, and an immutable selection hash. The view retains complete canonical snippets needed for target atoms, identities, relations, and verse anchors; it never clips text, and fails closed when required coverage cannot fit the total or per-verse bounds. The Spark schedule uses exact `gpt-5.3-codex-spark` at medium effort; Luna runs ten minutes later at exact `gpt-5.6-luna` low effort only after an eligible Spark model failure or authenticated missed/stale primary slot for that pair. A candidate is restricted to its work-item ID, fact briefs, and verse drafts. Submit verifies the work item plan version and current ledger lease, validates source binding/facts/prose, and stages an immutable unreviewed chunk. Assemble emits a schema-validated per-chapter review handoff that binds its winning model, automation, work item, source hashes, and ledger event; neither generation task can change the private manifest or publish. Review preparation remains ignored staging, and only an explicit approved all-assertions-true review may apply canonical runtime/audit/library changes before later `mhc:sync-latest` publication.
...
The controller requires `codex login status` to report a ChatGPT login and refuses an API-key state. Each request starts with exact `gpt-5.3-codex-spark`; scheduled `mhc:ensure` gives it one child process, and one eligible model-execution failure can invoke exact `gpt-5.6-luna` once with `model_reasoning_effort=low`. It performs no hidden per-model process retries. If both model attempts fail, the controller preserves their classifications and requires the verified complete-commentary link rather than a generated shard; the independently researched study may continue. Deterministic request, source, checksum, schema, security, repository, review, and publication failures never fall back. Sol, Terra, and every unspecified model are blocked. Worker invocations are noninteractive, ephemeral, read-only, sequential, standard-speed, and disable both multi-agent feature flags. Each child receives a unique writable SQLite home because the scheduler cannot write the parent Codex database. Before spawn, the controller creates a schema-only state database from `sqlite_master`, SQLx migration metadata, and a completed rollout-backfill marker; it never copies thread, rollout, project, prompt, or user-content rows, locks the directory/file to `0700`/`0600`, and deletes that exact temporary directory after the child closes. Any schema/bootstrap incompatibility is a controller-runtime failure and cannot consume the Luna attempt. Actual Spark/Luna provenance is recorded per fact brief, writer, runtime, reading, catalog, receipt, and review audit; immutable mixed-model artifacts remain valid. Final JSON is written to an explicit fingerprinted private path and validated after every job.
...
The same immutable reading bytes are merged into `stores/library/plans/<plan-key>/` and indexed by `mhc-library-catalog/v1`. `stores/library/current.json` points to the complete current-plan catalog and hashes its exact bytes. A later one-reading request replaces only that reading's catalog pointer; it never drops other retained readings or deletes an older content-addressed version. Reading objects are written first, then the catalog, pointer, and request-window manifest are replaced atomically. An interrupted or unavailable worker therefore leaves the last complete catalog/pointers readable. Native approved review application also runs the deterministic reviewed-library finalizer after its immutable canonical transaction commits. The finalizer validates the approved canonical audit/runtime bindings and can recover a legacy canonical-only admission with `npm run mhc:native:review:finalize -- --reading <readingId>`; it invokes no model and never edits the original review transaction.

Before a scheduled native reviewer opens a retained handoff, it runs `npm run mhc:native:review:work-order`. This read-only command selects at most one plan-bound handoff and reports only safe lane, reading, action/state/stage/code, and prior-manifest state. It recognizes a completed handoff only when the exact canonical committed transaction, current checksum-bound library artifact, attached local metadata, and manifest-backed reading all agree. A committed gap routes to deterministic finalization, attachment, or publication recovery; an exact approved-but-uncommitted binding routes to apply resumption; an uncommitted valid handoff routes to direct review; stale, tampered, or ambiguous state fails closed. Retained staging presence alone is never evidence that review is pending.
...
The controller verifies the catalog pointer, catalog hash/schema, immutable reading hash/schema, chapter runtime, and exact requested plan membership before treating a reading as available. For `mhc:ensure`, it resolves targets from the complete active calendar only after verifying that the fixture-backed prepared plan is its exact ordered prefix at the request plan version; an out-of-prefix request may contain only the immediate next active-calendar reading. The lookup does not extend that prefix or move a manifest. It invokes one Spark child only for missing or invalid targets, with one Luna-low child after an eligible Spark model-execution failure, merges successful results into the durable catalog without replacing the latest rolling-window manifest, verifies the complete requested set again, and writes `mhc-ensure-result/v1`. `mhc:ensure` rejects `--max-retries` so the scheduled exact-once route cannot silently expand. If both model attempts fail, the verified full-commentary link remains the Henry continuation and prior stored/published content remains intact. If every reading already exists, no model call occurs.
