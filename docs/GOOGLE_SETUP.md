...
The unchanged production URL currently serves confirmed-working immutable version 19. Version 20 (`8ef4415179bf0447`) was rolled back after Dustin's installed iPhone remained at the literal HTML **Starting…** state, which occurs before the first storage or server operation. Recovery version 21 (`898d9f53923bd23a`) is on the separate canary URL pending the phone release gate. It starts the core reader immediately, preserves IndexedDB schema version 4, loads live highlights as an optional post-core module, and adds bounded startup watchdogs. Version 19 established local-first rendering of an identity-bound saved calendar before the slow full bootstrap, followed by a lightweight identity/allowlist/enrollment/scopes/manifest check and a batched seven-reading refresh. Versions 16–18 remain useful incident history: they showed that commentary-schema rejection, ordinary stale-URL behavior, and shell size alone did not explain the prior iPhone stalls. Server enrollment, private Drive content, ESV retrieval, comments, scopes, execution identity, deployment access, and sharing remain unchanged; the restricted Sheet retains its frozen `highlight-events` tab. The server accepts commentary schemas v1, v2, and v3 while active substantive readings use v3.
