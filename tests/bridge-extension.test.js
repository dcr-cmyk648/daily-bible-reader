...
test("the bridge may append exactly the reading entering the Detroit T+7 window", () => {
  ...
  assert.equal(authorizedBridgeSourceDay({plan, appConfig, today: "2026-08-15"}), 68);
  const result = buildBridgeExtension({plan, appConfig, referencePlan, metrics, sourceDay: 68, today: "2026-08-15"});
  assert.equal(result.entry.readingId, "CC-Y3Q4-D068");
  assert.deepEqual(result.entry.passages, [
    {bookId: "2PE", chapter: 3, verseCount: 18}
  ]);
  assert.equal(result.plan.entries.length, plan.entries.length + 1);
  assert.equal(result.appConfig.testingReadingIds.at(-1), "CC-Y3Q4-D068");
});
...
test("the bridge cannot skip ahead or exceed the seven-day authorization", () => {
  ...
  const referencePlan = json("config/reference-plans/celebration-y3q4.json");
  const metrics = json("config/reference-plans/celebration-y3q4-chapter-metrics.json");
  assert.throws(() => buildBridgeExtension({plan, appConfig, referencePlan, metrics, sourceDay: 69, today: "2026-08-15"}), /append exactly/);
  assert.throws(() => buildBridgeExtension({plan, appConfig, referencePlan, metrics, sourceDay: 68, today: "2026-08-14"}), /T\+7 horizon/);
});
