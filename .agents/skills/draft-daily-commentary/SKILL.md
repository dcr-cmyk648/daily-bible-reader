...
The exact authenticated `gpt-5.3-codex-spark` model may be used as a drafting worker after the main task has finished the source inventory and prepared a paraphrased, rights-safe packet with no Scripture or raw source text. Invoke one worker sequentially, read-only, with multi-agent features disabled and a schema that binds the work-order and reading IDs. Preserve the raw output for audit. Spark may not browse, choose scope, turn snippets into consultation, approve its output, publish, or generate a second reading. The main task must verify every citation and remove all workflow-facing prose before validation.
...
Write for expert Christian readers under the repository's confessional stance. Produce one coherent, claim-cited main article rather than serial commentary notes; custom deep-study headings; a one- or two-paragraph orientation; one practical sentence; and an auditable source set. Present major contrary views only when influential and relevant, distinguish evidence from anti-supernatural premises, and assess them concisely.
