...
Accept either one legacy `commentary-work-order/v1` produced by `scripts/content-automation.mjs work-order`, one `rolling-study-work-order/v1` produced by `scripts/rolling-study-work-order.mjs`, or one `protocol-backfill-work-order/v1` produced by `scripts/protocol-backfill-work-order.mjs`. Every packet authorizes one reading only. The legacy order is draft-only. A rolling `prepare_publish` order or a protocol-backfill `refresh_review_publish` order authorizes private publication only after the primary task completes source review, content review, Henry review, validation, and content-first/manifest-last promotion. A protocol refresh may run only after the T+7 lane is ready; it preserves stable reading identity, comments/highlights, prior versions, and the newest reviewed Henry layer or verified fallback.

## 1. Verify the gate

1. Read repository `AGENTS.md`, `README.md`, and `PROJECT_STATE.md`.
2. Validate the packet against its exact schema and confirm its `reading` still matches the canonical active plan. A rolling order may name the one immediately appendable active-calendar entry when `planExtensionRequired` is true; run `npm run prefix:extend -- --today <YYYY-MM-DD>` and then revalidate the match.
3. For a legacy order, require its draft-only guards. For a rolling order, require `daysAhead: 7`, one reading, private-only content, primary review before publication, content-first/manifest-last promotion, `in_review` status, `daily-study-protocol/v1`, no stored ESV wording, no runtime AI, no comment access, and Spark limited to the Matthew Henry verse layer. For a protocol-backfill order, require `horizonPreparationFirst`, `alreadyReadOnly`, manifest-backed content, stable reading identity, preservation of comments/highlights/prior versions/newest reviewed Henry, and the same no-ESV/no-runtime-AI/no-comment-access limits.
...
Run the Matthew Henry preflight for the named passages. When exact reviewed atoms exist, use that source as the foundational confessional/pastoral pass and record its exact CrossWire edition. The controller tries exact Spark first; one Spark model-execution failure (including quota, availability, or process failure) may retry exact Luna once at low reasoning. Sol, Terra, and all other models are forbidden; deterministic request, source, checksum, schema, security, repository, review, and publication failures never fall back. If both model attempts fail, retain or attach the verified credential-free HTTPS link to the complete public-domain chapter commentary, bound to its real source-registry record, and continue the independently researched study. Label that fallback honestly; absence is never permission to invent a condensation. Build outward with independent grammatical, textual, historical, literary, canonical, theological, reception, and useful counterposition sources.
...
Write for expert Christian readers under the repository's confessional stance. Produce an executive main synthesis with one governing thesis and connected paragraphs, rather than serial commentary notes or compressed source coverage. State the best-supported interpretive point directly. Discuss a disagreement in the main synthesis only when the named positions materially change how the passage is read; otherwise move lesser, unresolved, technical, historical, or reception questions into the relevant custom deep-study section. Do not use vague recurring boilerplate such as “scholars debate” without naming a consequential dispute and explaining its interpretive stakes. Let the passage determine the movement, but normally establish the situation, explain its theological logic, identify its interpretive center, and land in the resulting response. Retain private claim markers and an auditable source set; the reader hides those markers from the short daily surface while the detailed synthesis displays its own numbered citations. Also supply a one- or two-paragraph orientation and one practical sentence. The takeaway should normally give the reader a specific attitude, temptation, motive, habit of attention, or response to watch in oneself that day. Use an outward action only when it is realistically achievable that day and directly warranted by the passage; reject vague assignments such as “fix an injustice.” Present major contrary views only when influential and relevant, distinguish evidence from anti-supernatural premises, and assess them concisely.

Record `generation.contentProtocolVersion: daily-study-protocol/v1` and exactly one `componentAssessments.historicalContext` result. When meaningful reading-specific archaeological or historical evidence exists, set it to `included` and include two distinct exact H3 sections in `synthesis.md`: `### Archaeological and historical context` for the concise preview and `### Archaeological and historical context — expanded study` for a separately researched dossier. Omit both when they would be filler; set the assessment to `not_material` only with a concise rationale and neither context layer. Once the concise preview is included for a prepared study, the expanded study is mandatory; never repeat or mechanically stretch the preview. Use materially fuller, independently drafted prose, at least two custom H4 topical headings, explicit evidence-versus-inference boundaries, inline claim citations, and a nearby bibliography. The preview and dossier must each use only their relevant source sets. Spark remains limited to the Matthew Henry verse layer and has no role in either context layer.
