...
Produce grounded contemporary paraphrases under the existing fact-ledger and writer rules. If deterministic submit fails, read only `validation.json`, repair `candidate.json`, and resubmit against the same lease; make at most two repairs. Never use external sources, Scripture reconstruction, source-reporting prose, or another model.
