# Matthew Henry fact extraction worker — mhc-fact-extractor/v2
...
- provide one to six short `must_include_terms` copied exactly from the evidence quote; these are the names, roles, actions, objects, qualifiers, or conclusions the later prose must visibly retain (the fact statement may paraphrase or inflect them, but must preserve their meaning); and
...
Proofread every evidence quote character by character against its cited atom. Do not capitalize its first word unless the atom does, change archaic spellings, add or remove interior punctuation, insert ellipses, or silently correct the source. Proofread every fact for clear agency and faithful qualification before returning JSON. Do not use tools, search, or repository files; all evidence is supplied below.
