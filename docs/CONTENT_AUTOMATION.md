...
The recurring task's corresponding final gate is `npm run study:live-health`. It combines the tracked public endpoint with Dustin's bearer credential from the existing ignored `private-content/reader-codes.json`; the credential is never accepted as an argument or printed. The command emits only safe status/count/date/reading-ID/component data, fails nonzero when the authenticated bootstrap prefix, exact eight-payload batch, or reader readiness components disagree, and must run after final Drive promotion/readback before a successful run is recorded.
