...
export function sparkModelUnavailable(result) {
  ...
  if (events.some(e => !allowed.has(e.type) || (e.type === 'item.completed' && e.item?.type !== 'error'))) return false;
  ...
  try {
    ...
    return refusal.type === 'error' && refusal.status === 400 &&
      ...
      refusal.error.message === "The 'gpt-5.3-codex-spark' model is not supported when using Codex with a ChatGPT account.";
  } catch { return false; }
}
