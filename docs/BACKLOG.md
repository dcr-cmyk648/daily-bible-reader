...
| ID | Status | Item | Acceptance boundary |
|---|---|---|---|
| `PLAN-001` | **queued; design inputs confirmed** | Build the new four-stream long-term plan beginning September 16, 2026, after the Celebration bridge. | One focused unit per day; confirmed 66-book Protestant canon; OT/NT proportional interweaving finishes both near together while normally keeping books contiguous; the larger OT stream may run on consecutive days and the NT ordinarily does not. Every book introduction is followed immediately by chapter 1. Sundays carry one proportionally allocated Psalm-or-Proverbs unit; historically associated Psalms are placed when well supported and otherwise retain canonical order; Proverbs uses coherent chunks. |
| `PLAN-002` | **partially confirmed** | Record remaining launch inputs before schedule generation. | Confirmed: canon, Detroit start date (2026-09-16), one-unit daily model, proportional OT/NT/Psalms/Proverbs pacing, Sunday allocation, book-introduction flow, and representative book verse. Remaining: exact stream/book order, Psalm placement decisions, Proverbs chunk boundaries, stream-balancing tolerance, future-lock behavior, browsing horizon, and recurring commentary length. |
