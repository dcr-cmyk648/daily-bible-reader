...
7. An unexpired private-content cache may paint immediately but is never version authority for the current and next reading. After authorization, both records must be re-fetched, validated, persisted over older revisions, and used to rerender readiness. A content-status or attached-commentary update may not require users to clear downloaded data manually.
...
The Nahum readiness incident exposed a separate stale-data class rather than an Apps Script startup failure: D057 remained `unreviewed`, and the priority warmer skipped a network read whenever a valid seven-day IndexedDB record existed. D057 is now `in_review` with its complete reviewed Henry layer attached, and the client regression-tests unconditional post-authorization revalidation for current and next. Commit `99622be` passed both GitHub workflows, exact live-file verification, and an authenticated 390×844 update/reload that replaced the stale D057 record, advanced the readiness gap to D058, loaded all 15 Henry notes, and produced no overflow or browser warnings/errors. The public release changes no service-worker storage schema, token transport, Apps Script backend, or ESV persistence rule, but the installed-iPhone update/reopen check remains required because priority-cache behavior changed.
