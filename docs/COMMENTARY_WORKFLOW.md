...
The synthesis is a prepublication artifact, never a runtime AI response. Work one reading at a time. The temporary Celebration bridge is a contiguous rolling window from `CC-Y3Q4-D054` through the reading exactly seven Detroit civil days ahead; on 2026-08-12 that boundary is D065. After the authorized initial fill, each scheduled run may prepare and privately publish only the one reading newly entering T+7. The Matthew Henry lane remains a source-specific foundational aid, not the broader synthesis. Reviewed layers attach only through the checksum-bound newest-artifact handoff. If Spark reports a quota/availability failure, do not switch models or write a replacement condensation; link the verified complete public-domain chapter commentary and label that limitation honestly.

Scheduled preparation uses the repo-local `$draft-daily-commentary` skill only after a validated work order. Legacy `commentary-work-order/v1` packets remain draft-only. The authorized rolling lane uses `scripts/rolling-study-work-order.mjs` and `rolling-study-work-order/v1`; it may prepare and privately publish exactly the T+7 reading after direct primary review, full validation, hash-bound Henry review, and content-first/manifest-last promotion. It cannot generate a second reading or move beyond the configured seven-day horizon.
...
3. One executive commentary synthesis in continuous prose: 220–600 words across two to six connected paragraphs. Establish one governing thesis, let each paragraph advance it, and move secondary but worthwhile questions into the detailed synthesis. Do not manufacture a numbered or titled point count or compress the source inventory into prose.
...
5. One-sentence, passage-specific focus for today (maximum 32 words): normally an achievable attitude, temptation, motive, habit of attention, or response to watch in oneself. Use outward action only when it is concrete, realistic today, and directly warranted by the passage; never assign vague work such as “fix an injustice.”
...
Publication metadata must support the three-page reader without generating separate content copies: `dailyIntroduction` feeds page 1; the server-delivered ESV passage feeds page 2 for chapter readings; `overview` feeds page 2 on a book-introduction day; and `commentarySummary`, `verseOfTheDay`, `practicalTakeaway`, `comprehensiveSynthesis`, and coverage/source metadata feed page 3. `verseOfTheDay` contains only `bookId`, `chapter`, and `verse`; validation requires it to fall inside that reading's configured passages. At runtime the browser isolates the exact verse from a valid live or policy-bounded cached ESV passage, shows ESV identification, the required notice, and an exact ESV.org passage link, and fails without substitution if Scripture is unavailable. Readings that exceed a display or cache limit use chapter or verse-range tabs and retain only compliant passage records. `commentarySummary.paragraphs` is one connected editorial unit, not a list of independent insight cards; its paragraphs may rely on one another and should use transitions. In `commentary/v3`, place `{{cite:source_id,source_id}}` immediately after supported claims in both the main and comprehensive syntheses. Every main marker ID must appear in that paragraph's `sourceIds`, and every declared main ID must appear in a marker. The main markers are private provenance and are stripped from the displayed executive synthesis; its collapsed bibliography remains available without interrupting the article. An end-to-end prepared comprehensive synthesis must cite every ID declared in `comprehensiveSynthesis.sourceIds`, may not cite an undeclared ID, and renders visible numbered links against its own nearby source list. Internal citation syntax must never appear in reader-visible prose. Comments remain attached once to the reading ID and are not duplicated per page.

The orientation, main synthesis, and takeaway use dedicated `dailyIntroduction`, `commentarySummary`, and `practicalTakeaway` metadata. Every notable commentary claim there must remain traceable through `sourceIds` and private citation markers to exact consulted registry records, even though the short daily surface does not interrupt its prose with numbers. The selected verse is traced directly to Scripture through its validated reference, not a commentary source ID.
...
Treat every displayed unit as independently useful within the day's reading. The orientation, the main synthesis as a whole, the takeaway, and each comprehensive section must identify enough of their own subject to stand alone. Paragraphs within the main synthesis should build on one another rather than behave as detachable mini-essays. Avoid backward/forward pointers that require another app section. If `contextReadingIds` identifies a genuinely relevant prior day, briefly restate the needed event, argument, or prophetic sequence and then show how today's text advances it. Make the payoff explicit: show how the material changes interpretation or a concrete response, while refusing generic application that could be attached to any passage.

The comprehensive synthesis is one coherent Markdown source document whose level-three headings are organic to that passage; do not force every reading through a fixed twelve-section template. At runtime, each heading becomes a separately collapsed deep-study section so the reader can scan the titles and open the relevant line of inquiry. This is where useful side questions, technical qualifications, disputed textual details, and broader reception material belong when they would interrupt the executive synthesis. The document should still cover the literary, historical, linguistic, theological, canonical, reception, disagreement, practical, and research-limit material that is genuinely useful, but its structure should follow the argument of the text. The research audit and complete source list have their own final disclosure.
...
Important claims carry source IDs. Orientation citations remain explicit links. Main-summary and takeaway markers stay in private metadata for claim-level review while the displayed daily prose remains uninterrupted; a collapsed bibliography identifies its source set. Deep-study markers render as unobtrusive numbers exactly where the supported claim appears and resolve to the detailed synthesis's nearby numbered list. Every cited ID must exist and be `consulted` or `included`; sources merely discovered/inaccessible cannot support a claim. Consensus language requires broad, substantially independent support. Use bounded wording such as “Across the sources consulted…” and identify source-set limits.
...
Run schema/referential/rights/safety checks. Human review covers accuracy, consistency with the stated editorial stance, fair representation of important alternatives, source breadth, traceability, quotation risk, length, readability, and whether summary prose could substitute for a source. It explicitly asks whether any conclusion gained authority mainly because a source assumed prophecy, miracle, or divine action was impossible or intrinsically unlikely. It also rejects a takeaway that is vague, implausibly large for one day, or aimed mainly at correcting other people; the default should give the reader a specific inward focus that can actually be practiced today. Set the review status and record reviewer/date/notes. Compute a content hash only after validation.
...
The private validator derives its exact reading set from the active plan. Every active reading requires `commentary/v3`, one comprehensive Markdown synthesis, matching content hashes, multiple independent sources in the main synthesis, every included source represented across the complete package, accurate single-source flags, and no likely stored ESV passage or unsafe raw HTML. An end-to-end prepared chapter additionally requires a configured ESV passage reference, orientation, coherent main synthesis, verse of the day, practical takeaway, comprehensive synthesis, traceable source metadata, and either complete reviewed Matthew Henry shards with exact cited public-domain source atoms for every configured chapter or the validated quota-only full-commentary link. ESV wording remains a server-fetched session resource and is never part of the stored preparation artifact.
...
The output is ignored at `private-content/bundles/celebration-bridge-review/`; its manifest remains `approvedForPublication: false`. Building the bundle is not publication. The rolling publication lane has explicit user approval for exactly its one T+7 reading after direct review and all gates. Upload data first, update the private manifest last, retain Drive revision history, and verify exact bytes and restricted sharing. Regenerating one reading must not require rebuilding unrelated readings.
...
The authorized recurring checkpoint is exactly T+7. Stop after that one reading even if later gaps are visible. Before validating or bundling a prepared study, resolve the checksum-bound current Henry catalog and attach its newest reviewed artifact; fail closed if a newer artifact remains unreviewed. A confirmed Spark quota/availability failure may use only the verified full-source link. The preserved Genesis calibration and this bridge inform the later launch plan, but the new long-term chronology remains outside this rolling authorization.
