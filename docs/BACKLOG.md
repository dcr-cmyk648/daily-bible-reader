...
| `AUTO-002` | **implemented, disabled by policy** | Let the main scheduled task instruct a bounded commentary-generation worker. | A stable `commentary-work-order/v1` packet names one plan entry and invokes `$draft-daily-commentary`. The tracked policy keeps generation disabled; output is ignored/unreviewed and cannot auto-publish. |
