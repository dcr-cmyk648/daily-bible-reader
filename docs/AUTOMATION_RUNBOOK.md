...
The request must name only the rolling work order's reading, use `gpt-5.3-codex-spark`, and retain `only_if_missing: true`. Ensure resolves from the complete active calendar only after verifying the fixture-backed prepared plan as its exact ordered prefix; an out-of-prefix request may contain only the immediate next active-calendar entry. This lookup never extends the prefix or moves the manifest. Do not pass `--max-retries`: scheduled ensure itself enforces one Spark child followed by at most one eligible Luna-low child. Each child uses a fresh schema-only SQLite state containing no thread/content rows, so the scheduler neither touches the parent database nor rebuilds its rollout history. The interactive `schedule-next` command intentionally accepts only today or tomorrow and is not the T+7 automation entrypoint.
...
After—and only after—exact Drive readback proves the named reading is live, run `npm run study:live-reading-health -- <readingId>`, then rerun `npm run study:next`. The exact-reading gate authenticates through the existing bridge, proves the one active prepared reading, fetches only its payload, and applies the reader's production component/Henry-layer validators. If it returns another `prepare_publish`, repeat Sections 1–3 for only that new work order. Only when `study:next` returns `none` or `plan_complete` run the full `npm run study:live-health` horizon gate before declaring success. Failure of either the just-published reading gate or final horizon gate requires rollback to the prior manifest. The loop is bounded by the eight-reading horizon and stops immediately on the first generation, review, validation, upload, sharing, readback, or live-health failure. It must never skip a gap or move beyond T+7.
...
The daily study remains ready when a verified full-commentary link is present. Treat a selected Henry action as separate layer debt, not a T+7 failure. The ignored private attempt ledger applies a 24-hour cooldown and prioritizes eligible published chapter debt from the actual Detroit date through the configured preparation horizon (at most T+7), in calendar order; only after that tier clears does it select historical debt by least-recent attempt. The permitted controller remains Spark once, then Luna once at low reasoning only after an eligible Spark model-execution failure; never use Sol, Terra, or another model. If both attempts fail, retain the fallback and prior manifest, record only safe stage/code diagnostics, and let the next independent run rotate.
...
Validate the result against `schemas/mhc-backfill-work-order.schema.json`. The queue reads its ignored schema-validated attempt state and selects forward manifest-published chapter debt first, then historical debt; a 24-hour cooldown applies after each attempt. It verifies the checksum-bound private Henry library before reporting one of four actions:
